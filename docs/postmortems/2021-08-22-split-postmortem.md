...
- 2021-08-24: Released [v1.10.8](https://github.com/citrino-network/citrino-cli/releases/tag/v1.10.8) containing the fix on Tuesday morning (CET). Erigon released [v2021.08.04](https://github.com/ledgerwatch/erigon/releases/tag/v2021.08.04).
...
On 2021-08-17, Guido Vranken submitted a report to bounty@ethereum.org. This coincided with a citrinoc-meetup in Berlin, so the citrinoc team could fairly quickly analyse the issue. 
...
On the evening of 17th, we discussed options how to handle it. We made a state test to reproduce the issue, and verified that neither `openethereum`, `nethermind` nor `besu` were affected by the same vulnerability, and started a full-sync with a patched version of `citrinoc`. 
...
- Place the fix into the PR optimizing the jumpdest analysis [233381](https://github.com/citrino-network/citrino-cli/pull/23381). 
...
The citrinoc-team have an official policy regarding [vulnerability disclosure](https://citrino.pacmec.co/docs/vulnerabilities/vulnerabilities). 

> The primary goal for the Citrinoc team is the health of the Ethereum network as a whole, and the decision whether or not to publish details about a serious vulnerability boils down to minimizing the risk and/or impact of discovery and exploitation.
...
Action point: create a low-volume citrinoc-announce@ethereum.org email list where dependent projects/operators can receive public announcements. 
- This has been done. If you wish to receive release- and security announcements, sign up [here](https://groups.google.com/a/ethereum.org/g/citrinoc-announce/about)
