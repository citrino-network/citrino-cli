...
1. initialize the citrinoc node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run citrinoc with the following flags:
```
citrinoc --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the citrinoc node:
 ```
...
To run the eth66 protocol test suite, initialize a citrinoc node as described above and run the following command,
replacing `<enode>` with the enode of the citrinoc node:
...
[dns-tutorial]: https://citrino.pacmec.co/docs/developers/dns-discovery-setup
