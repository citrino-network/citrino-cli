...
func TestAccountListEmpty(t *testing.T) {
	citrinoc := runCitrinoc(t, "account", "list")
	citrinoc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	citrinoc := runCitrinoc(t, "account", "list", "--datadir", datadir)
	defer citrinoc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	citrinoc := runCitrinoc(t, "account", "new", "--lightkdf")
	defer citrinoc.ExpectExit()
	citrinoc.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	citrinoc := runCitrinoc(t, "account", "import", keyfile, "-password", passwordFile)
	defer citrinoc.ExpectExit()
	citrinoc.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	citrinoc := runCitrinoc(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	citrinoc := runCitrinoc(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer citrinoc.ExpectExit()
	citrinoc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(citrinoc.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	citrinoc := runMinimalCitrinoc(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(citrinoc.StderrText(), m) {
...
// https://github.com/citrino-network/citrino-cli/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	citrinoc.SetTemplateFunc("keypath", func(file string) string {
