...
import (
	...
	"github.com/citrino-network/citrino-cli/crypto/bls12381"
)
...
func FuzzCrossPairing(data []byte) int {
	...
	// compute pairing using citrinoc
	...
	if !(bytes.Equal(cResult.Marshal(), bls12381.NewGT().ToBytes(kResult))) {
		panic("pairing mismatch gnark / citrinoc ")
	}
...
func FuzzCrossG1Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), g1.ToBytes(&kp))) {
		panic("G1 point addition mismatch gnark / citrinoc ")
	}
...
func FuzzCrossG2Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), g2.ToBytes(&kp))) {
		panic("G2 point addition mismatch gnark / citrinoc ")
	}
...
func FuzzCrossG1MultiExp(data []byte) int {
	var (
		input        = bytes.NewReader(data)
		citrinocScalars  []*big.Int
		gnarkScalars []fr.Element
		citrinocPoints   []*bls12381.PointG1
		gnarkPoints  []gnark.G1Affine
	)
	// n random scalars (max 17)
	for i := 0; i < 17; i++ {
		// note that citrinoc/crypto/bls12381 works only with scalars <= 32bytes
		...
		citrinocScalars = append(citrinocScalars, s)
		var gnarkScalar = &fr.Element{}
		gnarkScalar = gnarkScalar.SetBigInt(s).FromMont()
		gnarkScalars = append(gnarkScalars, *gnarkScalar)

		citrinocPoints = append(citrinocPoints, new(bls12381.PointG1).Set(kp1))
		gnarkPoints = append(gnarkPoints, *cp1)
	}
	if len(citrinocScalars) == 0 {
		return 0
	}
	// compute multi exponentiation
	g1 := bls12381.NewG1()
	kp := bls12381.PointG1{}
	if _, err := g1.MultiExp(&kp, citrinocPoints, citrinocScalars); err != nil {
		panic(fmt.Sprintf("G1 multi exponentiation errored (citrinoc): %v", err))
	}
	// note that citrinoc/crypto/bls12381.MultiExp mutates the scalars slice (and sets all the scalars to zero)
	...
	// compare result
	if !(bytes.Equal(cp.Marshal(), g1.ToBytes(&kp))) {
		panic("G1 multi exponentiation mismatch gnark / citrinoc ")
	}
...
func getG1Points(input io.Reader) (*bls12381.PointG1, *gnark.G1Affine, error) {
	...
	// marshal gnark point -> citrinoc point
	g1 := bls12381.NewG1()
	kp, err := g1.FromBytes(cpBytes)
	if err != nil {
		panic(fmt.Sprintf("Could not marshal gnark.G1 -> citrinoc.G1: %v", err))
	}
	if !bytes.Equal(g1.ToBytes(kp), cpBytes) {
		panic("bytes(gnark.G1) != bytes(citrinoc.G1)")
	}
...
func getG2Points(input io.Reader) (*bls12381.PointG2, *gnark.G2Affine, error) {
	...
	if err != nil {
		panic(fmt.Sprintf("Could not marshal gnark.G2 -> citrinoc.G2: %v", err))
	}
	if !bytes.Equal(g2.ToBytes(kp), cpBytes) {
		panic("bytes(gnark.G2) != bytes(citrinoc.G2)")
	}
